...
#ChN=c(0,74)
ChX=head(abs(pikes$X0), length(ChN))
...
N=c(0,17,23,74)
Sig=head(abs(pikes$sigma), length(N))

SigeV=line_select(N)

SigM=lm(Sig ~ SigeV + I(sqrt(SigeV)))
 # ChX ~ CheV + I(CheV**2) + I(CheV**3)
...
#plot(Ev_to_Sig(rel_lines$eV), h_, col='green', type='h')
print(ChN)
...
pikes$N=ChN
print(pikes)
#text(Ev_to_Sig(rel_lines$eV), h_2, col='dark red', lab=rel_lines$N)

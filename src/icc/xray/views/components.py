...
class TXRFNavigationToolbar(drawing.NavigationToolbar):
    toolitems = drawing.NavigationToolbar.toolitems + (
        ...
        (False, 'Subplots', 'Configure subplots', 'subplots.png',
         'configure_subplots', 'gtk-preferences'),
        (False, 'Save', 'Save the figure',
         'filesave.png', 'save_figure', 'gtk-convert'),
        (True,  'Channels', 'Explore channel counts',
         'filesave.png', 'explore_channels', 'gtk-color-picker'),
    )
...
@implementer(IPlottingView)
@ZC.adapter(mdli.IProject, rakeints.IView)
class PlottingView(View):
    ...
    def __init__(self, model=None, parent=None):
        ...
        fig = Figure(figsize=(5, 4), dpi=120,
                     subplotpars=matplotlib.figure.SubplotParams(
                         left=0.03, right=0.96, bottom=0.03, top=0.96)
                     )
        self.ui.fig = fig
    ...
    def paint_model(self, model, draw=True, conserve=True):
        ...
        if not model:
            ...
            for i, spec in enumerate(m):
                ...
                ax.axhline(y=0, xmin=0, xmax=1, color=(
                    0, 0, 0), alpha=0.3, linestyle='--')
                # ax.set_yticklabels([])
                # ax.set_xticklabels([])
                #spec['line2D'] = pl
    ...
    def on_click(self, event, data=None):
        ...
        if event.xdata and event.ydata:
            ...
            s = ' '.join([str(x) for x in [event.button, event.x,
                                           event.y, event.xdata, event.ydata]])

        local.msg_id = self.ui.sb.push(local.ctx_id, s)

        self.emit('spectrum-clicked', event.button, event.x,
                  event.y, event.xdata, event.ydata)
...
@implementer(rakeints.IProjectView)
@ZC.adapter(mdli.IProject, rakeints.IView)
class ProjectView(View):
    ...
    def __init__(self, model=None, parent=None):
        ...
        opt = _conf.add_option(
            'spectra_file_ext', default='.*:All Files', keys='app')
        self.FILE_PATTERNS = [e.split(':') for e in opt.get().split('|')]

        self.active_view = ZC.getMultiAdapter(
            (self.model, self), IPlottingView)
        self.active_view.connect(
            'spectrum-clicked', self.on_plot_spectrum_clicked)
        ...
        self.ui.main_vbox.pack_start(
            self.active_view.ui.main_frame, True, True, 0)
        ...
        self.project_tree_selection.connect(
            'changed', self.on_project_tree_selection_changed)
    ...
    def p_thread_tasks(self, tasks):
        if self.p_thread == None or not self.p_thread.is_active():
            self.p_thread = proc.Parameters(
                self.active_view.model[0], self.active_view)  # adapter ??
    ...
    def set_model(self, model=None):
        ...
        for name, sd in self.model.spectral_data.items():
    ...
    def set_element_list(self, spec=None, sp_and_no=None):
        ...
        return  # FIXME
        for el in list(spec.elements.values()):
    ...
    def interp_path(self, path):
        ...
        filename, sp = list(self.model.spectral_data.items())[file_no]
        if lp == 2:
    ...
    def on_row_activated(self, tree_view, path, column, data=None):
        ...
        if lp == 2:
            ...
            self.emit('spectrum-clicked', filename,
                      sp.data[spec_no], (sp, spec_no))
    ...
    def on_paned_notify(self, paned, spec, data=None):
        if spec.name == 'position':
            ...
            # recursion breaks due to position property: it can be unchanged.
            [p.set_position(pos) for p in self.ui.hpaned_list if p != paned]
...
@implementer(IPeriodicTableView)
class PeriodicTableWindow(View):
    ...
    def __init__(self, model=None):
        ...
        # Last cursor clicked here. Modified by a superior widget.
        self.cursor_clicked = None
